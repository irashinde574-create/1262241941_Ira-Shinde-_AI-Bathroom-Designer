...
def run_stage_b(style, top_bundles):
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            config={
                ...
                # gemini-3.6-flash has "thinking" on by default, and those
                # tokens count against max_output_tokens — that was eating
                # the whole budget and truncating the actual JSON answer
                # before it could finish. This is a simple classification
                # task, so thinking isn't needed; turn it off.
        ...
        print("=== STAGE B: LLM did not return valid JSON ===")
        print("RAW RESPONSE WAS:", repr(raw_text))
        return {
            "status": "error",
    ...
    if not isinstance(selected_index, int) or not (0 <= selected_index < len(top_bundles)):
        print("=== STAGE B: invalid bundle index from LLM ===")
        print("PARSED WAS:", parsed)
        return {
            "status": "error",
