...
from stage_a import run_stage_a, load_catalog, resolve_layout
from layouts import AUTO, LAYOUTS
...
def design():
    ...
    layout_choice = (data.get("layout") or AUTO).strip()
    if layout_choice != AUTO and layout_choice not in LAYOUTS:
        return jsonify({"status": "input_error", "message": "Unknown layout choice."}), 400
    ...
    layout_id = stage_a_result["layout_id"]

    if stage_a_result["status"] == "room_infeasible":
        message = stage_a_result["reason"]
        if not stage_a_result["layout_auto"]:
            message += " Try the Auto layout setting — another arrangement may fit."
        return jsonify({"status": "room_infeasible", "message": message})
    ...
    if stage_a_result["status"] == "no_valid_products":
        ...
        return jsonify({
            ...
            "message": f"No products fit the available space for: {', '.join(empty_cats)}. "
                       f"Try a larger room or a different layout.",
        })
    ...
    svg_markup = get_floorplan_svg(CATALOG_PATH, room_width_ft, room_depth_ft, style,
                                     final_bundle, total_price, layout_id)
    scene_3d = get_3d_scene_data(CATALOG_PATH, room_width_ft, room_depth_ft, final_bundle, layout_id)
    ...
    return jsonify({
        ...
        "layout": {
            "id": layout_id,
            "label": stage_a_result["layout_label"],
            "auto": stage_a_result["layout_auto"],
            "reason": stage_a_result["layout_reason"],
        },
...
def refine():
    ...
    # The layout the design was generated with (already resolved from "auto"
    # by /api/design). Fall back to resolving from the room if a client omits it.
    layout_id = (data.get("layout_id") or "").strip()
    if layout_id not in LAYOUTS:
        layout_id = resolve_layout(load_catalog(CATALOG_PATH), room_width_ft, room_depth_ft)["layout_id"]
    ...
    result = apply_refinement(CATALOG_PATH, room_width_ft, room_depth_ft, style,
                               budget_inr, current_bundle, target_category, direction, layout_id)

    if result["status"] != "ok":
        return jsonify(result)

    svg_markup = get_floorplan_svg(CATALOG_PATH, room_width_ft, room_depth_ft, style,
                                     result["bundle"], result["total_price"], layout_id)
    scene_3d = get_3d_scene_data(CATALOG_PATH, room_width_ft, room_depth_ft, result["bundle"], layout_id)
