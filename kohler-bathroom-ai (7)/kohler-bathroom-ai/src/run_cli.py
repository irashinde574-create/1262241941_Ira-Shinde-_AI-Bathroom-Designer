...
def main():
    ...
    layout = input(f"Layout ({AUTO} / {' / '.join(LAYOUTS)}) [{AUTO}]: ").strip() or AUTO
    if layout != AUTO and layout not in LAYOUTS:
        print(f"❌ Unknown layout {layout!r}.")
        return

    print("\nRunning constraint checks and budget optimization...\n")

    result = run_stage_a(CATALOG_PATH, room_width_ft, room_depth_ft, budget_inr, style, layout=layout)

    status = result["status"]
    print(f"Layout: {result['layout_label']}" + (" (chosen automatically)" if result["layout_auto"] else ""))
    if result["layout_reason"]:
        print(f"  {result['layout_reason']}")
    print()
