...
1. Checks whether the room can physically fit a toilet, vanity, and shower
   zone (single-wall, L-shaped or opposite-wall layout — see assumptions below)
...
using one of three layout templates, defined in `src/layouts.py`:

| Layout id | Arrangement | Suits |
|---|---|---|
| `single_wall` | Toilet, vanity and shower in one row along the back wall | Long, shallow rooms |
| `l_shaped_corner_shower` | Shower in a back corner, vanity beside it, toilet on the adjoining side wall | Roughly square rooms |
| `opposite_walls` | Toilet and vanity on the back wall, shower in the front corner opposite | Rooms deeper than they are wide |

The customer can pick a layout in the form, or leave it on **Auto**: layouts
are then tried in an order chosen from the room's width : depth ratio, and the
first one that is geometrically feasible *and* leaves room for at least one
catalog toilet is used. If none fits, the single-wall layout's "room too
small" message is reported. The layout in use is shown above the floorplan.

Every zone is placed against a wall using wall-local coordinates (distance
along the wall, distance out from it), so Stage A's fit checks, the 2D
floorplan and the 3D scene all read the same placement. Adding a layout means
adding one `evaluate_` and one `plan_` function in `layouts.py`; the
renderers don't change. `tests/test_layouts.py` checks, across a sweep of room
sizes, that every toilet Stage A accepts for a layout can actually be placed
without leaving the room or overlapping another zone.

Two simplifications worth knowing about: clearance areas are reserved
exclusively (they are never allowed to overlap, even where real planning
guidance would let two fixtures share a walkway), which makes
`opposite_walls` conservative about room depth; and the door, windows and
irregular room shapes are not modeled.
...
├── tests/
│   └── test_layouts.py           # Layout feasibility / placement / 2D-3D agreement tests
└── src/
    ...
    ├── layouts.py                 # Layout templates (single wall, L-shaped, opposite walls)
...
- [ ] Irregular room shapes and door/window placement (layouts are currently three rectangular-room templates)
- [ ] Shared walkway clearance between facing fixtures (would let `opposite_walls` fit shallower rooms)
